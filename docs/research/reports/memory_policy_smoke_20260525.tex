...
here is not to claim a final winner. It is to verify that the five memory
...
NVIDIA runs. In this smoke sweep, validation loss is essentially tied across
the memory policies, while dense attention is much faster in plain PyTorch.
\end{abstract}
...
context window. For this Mac sweep, we only train for 2048 tokens so the run
finishes quickly and the plots are easy to inspect.

\begin{table}[t]
\centering
\small
\caption{Shared setup for the Mac smoke sweep.}
\label{tab:setup}
\begin{tabular}{@{}ll@{}}
\toprule
Item & Value \\
