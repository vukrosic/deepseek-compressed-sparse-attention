...
\title{\vspace{-1.1em}\bfseries 10 Forgetting Attention Mechanisms}
...
\begin{abstract}
We collect ten forgetting and memory policies for causal attention and
...
18M-class transformer under a fixed one-GPU time budget. In this small,
...
hierarchical summaries, surprise retention --- cluster at the top.
\end{abstract}
...
policies do not beat dense or local. Their value here is diagnostic:
the rules are correct, runnable, and ranked, so the next iteration
knows which paths are worth fusing or rerunning at larger scale.
